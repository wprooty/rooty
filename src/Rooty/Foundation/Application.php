...
use Rooty\Contracts\Foundation\CachesConfiguration;
...
class Application extends Container implements ApplicationContract, CachesConfiguration
{
    ...
    protected static $defaultFacadeAliases = [
        ...
        \Rooty\Support\Facades\Arch::class => 'Arch',
    ];
     ...
     *
     * @var array<string, \Rooty\Support\ServiceProvider>
     */
     ...
     *
     * This method attempts to use the WordPress timezone setting if available.
     * It supports both named regions (e.g. "Europe/Paris") and raw offsets
     * (e.g. "+03:30"). Offsets are normalized into valid IANA timezone identifiers
     * using CarbonTimeZone. If no valid timezone can be resolved, it falls back to "UTC".
     *
     * @return string The resolved timezone identifier (e.g. "Europe/Paris", "UTC", "America/St_Johns").
     */
    public function resolveTimezone(): string
    {
        ...
        try {
            $tz = CarbonTimeZone::create($name);
            $region = $tz->toRegionTimeZone();

            return $region ? $region->getName() : $tz->getName();
        } catch (Exception $e) {
            return 'UTC';
        }
    }
    ...
    /**
     * Handle the incoming Arch command.
     *
    ...
    public function registerConfiguredProviders()
    {
        ...
        [$illuminate, $nonIlluminate] = $allProviders->partition(
            fn ($provider) => str_starts_with($provider, 'Illuminate\\')
        );

        [$rooty, $others] = $nonIlluminate->partition(
            fn ($provider) => str_starts_with($provider, 'Rooty\\')
        );

        $providers = new Collection([$illuminate, $rooty, $others]);

        $providers->splice(2, 0, [$this->make(PackageManifest::class)->providers()]);

        $providers = $providers->collapse();
     ...
     *
     * @param  \Rooty\Support\ServiceProvider|string  $provider
     * @param  bool  $force
     * @return \Rooty\Support\ServiceProvider
     */
     ...
     *
     * @param  \Rooty\Support\ServiceProvider|string  $provider
     * @return \Rooty\Support\ServiceProvider|null
     */
     ...
     *
     * @param  \Rooty\Support\ServiceProvider  $provider
     * @return void
     */
     ...
     *
     * @param  \Rooty\Support\IlluminateServiceProvider  $provider
     * @return void
     */
    protected function bootProvider(IlluminateServiceProvider $provider)
    {
